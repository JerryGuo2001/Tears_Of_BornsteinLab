// public/style.js
// Visuals: background draw modes, sprite/blob with bob + shadow,
...
function drawGroundShadow(ctx, x, y, worldW, intensity=1){
  ...
  ctx.beginPath();
  if (ctx.ellipse) ctx.ellipse(x, y + yOff, rx, ry, 0, 0, Math.PI*2);
  else { ctx.translate(x, y+yOff); ctx.scale(rx/(worldW*0.5), ry/(worldW*0.5)); ctx.arc(0,0,worldW*0.5,0,Math.PI*2); }
  ctx.fill();
  ctx.restore();
}
...
class AnimatedSheetSprite {
  ...
  draw(ctx, x, y, worldW, state={}) {
    ...
    const dw = worldW, dh = dw * (frameH / frameW);
    const sx = frame * frameW, sy = rowIdx * frameH;
    ...
    ctx.drawImage(this.img, sx, sy, frameW, frameH, -dw*0.5, -dh*0.5, dw, dh);
    ctx.restore();
  }
}
...
class BlobDude {
  constructor({ color = "#3b82f6", radius = 16, speedMul = 1 } = {}) {
    this.color = color; this.radius = radius; this.speedMul = speedMul;
  }
  draw(ctx, x, y, worldW, state={}) {
    const R = worldW / 2, t = state.t || 0;
    ...
    const eyeOffsetX = R * 0.34, eyeOffsetY = -R * 0.1, eyeR = R * 0.13;
...
export async function loadStyle(theme) {
  ...
  const drawBackground = (ctx, mapSize) => {
    if (!bgImg) return;
    const mode = theme?.background?.mode || "cover";  // "stretch" | "cover" | "contain" | "tile"
    ...
    if (mode === "tile") {
      ...
      ctx.fillStyle = pat; ctx.fillRect(0,0,mapSize,mapSize);
    } else {
      let dx = 0, dy = 0, dw = mapSize, dh = mapSize;
      if (mode === "cover" || mode === "contain") {
        const iw = bgImg.width, ih = bgImg.height;
        const s = mode === "cover" ? Math.max(mapSize/iw, mapSize/ih) : Math.min(mapSize/iw, mapSize/ih);
        dw = iw * s; dh = ih * s; dx = (mapSize - dw)/2; dy = (mapSize - dh)/2;
      }
  ...
  // Player sprites
  let sheetImg = null, sheetCfg = null;
  ...
  const getPlayerSprite = (seatIndex=0, color="#3b82f6", radius=16) => {
    if (cache.has(seatIndex)) return cache.get(seatIndex);
    let sprite;
    if (sheetImg && sheetCfg) sprite = new AnimatedSheetSprite(sheetImg, sheetCfg);
    else {
      const palette = [color, "#10b981", "#f59e0b", "#e879f9"];
      sprite = new BlobDude({ color: palette[seatIndex % palette.length], radius, speedMul });
    }
    cache.set(seatIndex, sprite);
    return sprite;
  };
  ...
  if (theme?.music?.src) {
    ...
    el.loop   = true;
    el.preload = "auto";
    el.volume = clamp(theme.music.volume ?? 0.7, 0, 1);
...
export const THEMES = {
  grass: {
    ...
    player: { /* blob fallback or provide sheet */ }
  },
  isaacish: {
    animSpeed: 0.8,
